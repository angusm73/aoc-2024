...
class DoorCodeSolver
  ...
  def min_length_DPAD(sequence, depth)
    ...
    return @cache[[sequence, depth]] if @cache[[sequence, depth]]
...
puts solver.calculate_total_complexity(helper.input_lines.map(&:chomp), 25)
